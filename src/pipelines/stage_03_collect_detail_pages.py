...
def _should_skip_row(row, existing_outputs):
    """Check if we should skip processing this row based on existing outputs."""
    detail_url = row.get('detail_url', '').strip()
    if not detail_url:
        return True, "empty url"

    # Check if we already have output for this URL
    for existing in existing_outputs:
        if existing.get('detail_url') == detail_url:
            # If we already have this URL processed, skip it
            # Only reprocess if the input data is significantly newer
            input_collected = row.get('collected_at', '')
            output_collected = existing.get('collected_at', '')

            if input_collected and output_collected:
                # ISO formatted timestamps compare correctly as strings.
                if input_collected <= output_collected:
                    return True, "already processed"
            else:
                # If no timestamps, assume already processed
                return True, "already processed"

    return False, None


def run(target='all', input_path=None, run_id=None, headless=None, limit=0, incremental=True):
    ...
    with sync_playwright() as p:
        ...
        try:
            for task_name in selected:
                ...
                # Load existing outputs for incremental processing
                existing_outputs = []
                if incremental:
                    # Try to find the latest existing output
                    existing_index_dir = Path('data/stage/03_detail_index')
                    if existing_index_dir.exists():
                        existing_files = list(existing_index_dir.rglob(f'{task_name}.jsonl'))
                        if existing_files:
                            # Sort by modification time, get the latest
                            latest_file = max(existing_files, key=lambda f: f.stat().st_mtime)
                            try:
                                existing_outputs = _read_jsonl(latest_file)
                                logger.info('loaded %d existing outputs from %s', len(existing_outputs), latest_file)
                            except Exception as e:
                                logger.warning('failed to load existing outputs: %s', e)

                logger.info('start task=%s input=%s raw_dir=%s incremental=%s', task_name, input_path, raw_dir, incremental)
                ...
                for idx, row in enumerate(rows, start=1):
                    ...
                    if incremental:
                        should_skip, reason = _should_skip_row(row, existing_outputs)
                        if should_skip:
                            logger.info('skip detail idx=%s (%s)', idx, reason)
                            skipped += 1
                            continue
...
def main():
    ...
    parser.add_argument('--target', default='all', choices=['all', 'xuezhi_major', 'xuezhi_career'])
